        ...
        <div class="card-body">
          <div class="row">
            <div class="col-md-8">
              ...
              <p class="text-muted">{{ $exam->subject }} | Class: {{ $class->name }} | Department: {{ $department->name ?? 'N/A'}}</p>
              ...
              <div class="card border-primary mb-3">
                <div class="card-body">
                  ...
                  <div class="progress mb-3" style="height: 25px;">
                    @php
                      $percentage = $exam->total_questions > 0 ? ($questions->count() / $exam->total_questions) * 100 : 0;
                    @endphp
                  ...
                  @else
                    <span class="badge bg-warning"><i class="fas fa-exclamation-circle"></i> {{ $exam->total_questions - $questions->count() }} remaining</span>
                  @endif
                </div>
              </div>
                  ...
                  <ul class="list-unstyled">
                    ...
                    <li class="mt-2">
                      ...
                      @if($exam->show_explanations)
                        <span class="badge bg-success">Show Explanations</span>
                      @else
                        <span class="badge bg-secondary">Hide Explanations</span>
                      @endif
                    </li>
                  </ul>
                </div>
        ...
        <div class="card-body">
          <div class="d-flex justify-content-between align-items-center mb-3">
            <h5 class="card-title">
              Questions 
              @if($questions->count() >= $exam->total_questions)
                <span class="badge bg-success">{{ $questions->count() }}/{{ $exam->total_questions }} Complete</span>
              @else
                <span class="badge bg-warning">{{ $questions->count() }}/{{ $exam->total_questions }}</span>
              @endif
            </h5>
            @if($questions->count() < $exam->total_questions)
              <a href="{{ route('questions.create', [$class->id, $exam->id]) }}">
            ...
            @else
              <div>
                <span class="badge bg-success mr-2">All questions added!</span>
                @if(!$exam->is_published)
                  <button class="btn btn-success btn-sm" onclick="publishExam('{{ route('exams.publish', [$class->id, $exam->id]) }}')">
                ...
                <tbody>
                  @foreach($questions as $question)
                    <tr>
                      ...
                      <td>
                        <a href="{{ route('questions.edit', [$class->id, $exam->id, $question->id]) }}">
                          <button class="btn btn-warning btn-sm" title="Edit Question">
                            <i class="fas fa-edit"></i>
                          </button>
                        </a>
                        <button class="btn btn-danger btn-sm" onclick="confirmDelete('{{ route('questions.destroy', [$class->id, $exam->id, $question->id]) }}', '{{ addslashes($question->question_text) }}')" title="Delete Question">
          ...
          @else
            <div class="alert alert-warning">
              ...
              You need to add <strong>{{ $exam->total_questions }} questions</strong> to this practice exam before you can publish it.
              <a href="{{ route('questions.create', [$class->id, $exam->id]) }}" class="btn btn-primary btn-sm mt-2">
      ...
      <div class="card mt-4">
        <div class="card-body">
          <div class="d-flex gap-2">
            <a href="{{ route('exams.edit', [$class->id, $exam->id]) }}">
              <button class="btn btn-warning">
                <i class="fas fa-edit"></i> Edit Exam Details
              </button>
            </a>
            @if($questions->count() >= $exam->total_questions && !$exam->is_published)
              <button class="btn btn-success" onclick="publishExam('{{ route('exams.publish', [$class->id, $exam->id]) }}')">
                <i class="fas fa-check"></i> Publish Exam
              </button>
            @endif
  ...
  <script>
    ...
    function publishExam(url) {
      Swal.fire({
        ...
        text: "Once published, students will be able to access this practice exam.",
        icon: "info",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
